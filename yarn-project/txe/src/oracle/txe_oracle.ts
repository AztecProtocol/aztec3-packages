...
export class TXE implements TypedOracle {
  ...
  async getPrivateContextInputs(
    ...
    if (blockNumber >= this.blockNumber) {
      this.logger.verbose(
        `Tried to request private context inputs for ${blockNumber}, which is greater or equal to current block of ${this.blockNumber}. Clamping to current block - 1.`,
      );
  ...
  private async executePublicFunction(args: Fr[], callContext: CallContext, isTeardown: boolean = false) {
    ...
    const publicDataWrites = result.avmProvingRequest.inputs.output.accumulatedData.publicDataWrites.filter(
      s => !s.isEmpty(),
    );
    await this.addPublicDataWrites(publicDataWrites);
